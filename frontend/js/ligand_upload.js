(function () {
  ...
  function validateLigandResnames(nFiles) {
    ...
    for (i = 0; i < nFiles; i++) {
      ...
      if (s.length > RESNAME_MAX) {
        return "配体 " + (i + 1) + " 残基名过长：最多 " + RESNAME_MAX + " 个字符";
      }
  ...
  /** 绑定标题旁可编辑残基名输入（外观同原 LIG 标签） */
  function bindResnameInput(inp, fallback) {
    if (!inp) return;
    inp.placeholder = fallback;
    inp.setAttribute("data-default", fallback);
    inp.addEventListener("click", function (e) {
      // 避免点标签时触发外层 label 打开文件选择
      e.preventDefault();
      e.stopPropagation();
      inp.focus();
    });
    ...
    inp.addEventListener("input", function () {
      ...
      if (v.length > RESNAME_MAX) v = v.slice(0, RESNAME_MAX);
  ...
  function updateRowMeta(row, index) {
    ...
    if (rnInp) {
      ...
      rnInp.setAttribute("aria-label", "配体 " + (index + 1) + " 残基名");
    }
  }
  ...
  function bindExtraRow(row, index) {
    ...
    var meta = LIG_META[index] || LIG_META[2];
    bindResnameInput(row.querySelector(".ligand-resname-input"), meta.tag);
  ...
  bindResnameInput(primaryResname, "LIG1");
  refreshAddButton();
